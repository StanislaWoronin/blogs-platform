...
export class SubscribeToBlogUseCase {
  ...
  async execute(userId: string, blogId: string): Promise<boolean> {
    const isExists = await this.integrationRepository.blogAndSubscriptionExists(userId, blogId)
    ...
    const subscribe = await this.integrationRepository.subscribeToBlog(blogSubscription)
