...
import {
  ...
  blogsForCurrentUser,
  createdBlog,
} from './helper/exect-blogger.model';
...
describe('e2e tests', () => {
  ...
  describe('Blogger router testing', () => {
    ...
    it('Create users', async () => {
      const [user0, user1, user2, user3, user4] =
      ...
      expect.setState({
        ...
        user3: user3.user,
        user4: user4.user,
        accessToken2: user0.accessToken,
      });
    });
    ...
    describe('blogger/users', () => {
      describe('Ban users', () => {
        it('Try ban without authorization', async () => {
          ...
          const url = getUrlForBanned(
            endpoints.bloggerController.users[''],
            user1.id,
          );
        ...
        it('Should ban three users', async () => {
          ...
          const url1 = getUrlForBanned(
            endpoints.bloggerController.users[''],
            user0.id,
          );
          const url2 = getUrlForBanned(
            endpoints.bloggerController.users[''],
            user2.id,
          );
          const url3 = getUrlForBanned(
            endpoints.bloggerController.users[''],
            user4.id,
          );
      ...
      describe('Return all banned users for blog', () => {
        it('Try get users without authorized', async () => {
          ...
          const url = getUrlWithId(
            endpoints.bloggerController.users.blogs,
            blog.id,
          );
        ...
        it('Get users with query parameters', async () => {
          ...
          const response1 = await request(server)
            .get(
              `${url}?sortBy=login&sortDirection=asc&pageNumber=1&pageSize=2`,
            )
            .auth(accessToken, { type: 'bearer' })
            .expect(200);
          ...
          expect(response1.body).toEqual({
            ...
            pageSize: 2,
            totalCount: 3,
            items: [bannedUser(user0), bannedUser(user2)],
          });

          const response2 = await request(server)
            .get(
              `${url}?sortBy=login&sortDirection=desc&pageNumber=2&pageSize=2`,
            )
            .auth(accessToken, { type: 'bearer' })
            .expect(200);
    ...
    describe('blogger/blogs', () => {
      describe('Return all comments for all post current user`s blog', () => {
        ...
        it('Create data', async () => {
          ...
          const [post0, post1, post2] = await factories.createPostsForBlog(
          ...
          const [post3] = await factories.createPostsForBlog(
      ...
      describe('Update exist blog by id', () => {
        ...
        it('Create data', async () => {
          const [accessToken1, accessToken2] =
            await factories.createAndLoginUsers(2);

          const blog1 = await factories.createBlogs(
            accessToken1.accessToken,
            1,
          );
          const blog2 = await factories.createBlogs(
        ...
        it('Try update blog with incorrect input data', async () => {
          ...
          const errorsMessages = getErrorMessage([
            'name',
            'description',
            'websiteUrl',
          ]);
        ...
        it('Update blog', async () => {
          ...
          expect(response.body.items[0].description).toBe(
            preparedBlog.newValid.description,
          );
          expect(response.body.items[0].websiteUrl).toBe(
            preparedBlog.newValid.websiteUrl,
          );
        });
      });
      ...
      describe('Return all blogs for current user', () => {
        ...
        it('Create data', async () => {
          ...
          const [blog0, blog1, blog2] = await factories.createBlogs(
            token.accessToken,
            3,
          );
        ...
        it('Should return blogs with pagination', async () => {
          ...
          const response2 = await request(server)
            .get(
              `${endpoints.bloggerController.blogs}?sortBy=name&sortDirection=asc&pageNumber=2&pageSize=2`,
            )
            .auth(accessToken, { type: 'bearer' })
            .expect(200);
          ...
          const response3 = await request(server)
            .get(
              `${endpoints.bloggerController.blogs}?sortBy=name&sortDirection=desc&pageNumber=1&pageSize=2`,
            )
            .auth(accessToken, { type: 'bearer' })
            .expect(200);
      ...
      describe('Create new post for specific blog', () => {
        ...
        it('Try create post without authorization', async () => {
          ...
          await request(server).post(url).send(preparedPost.valid).expect(401);
        });
      ...
      describe('Update exist post by id', () => {
        ...
        it('Try update not existing post', async () => {
          const { accessToken, blogId1 } = expect.getState();
          const randomId = randomUUID();
          const url = getUrlPostForSpecificBlog(
            endpoints.bloggerController.blogs,
            blogId1,
            randomId,
          );
